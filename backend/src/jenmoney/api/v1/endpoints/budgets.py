...
def create_budget(
    *,
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to calculate actual spending for budget: {str(e)}"
        )
...
def read_budgets(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to retrieve budget data: {str(e)}"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to process budget data: {str(e)}"
        )
...
def read_budget(*, db: Session = Depends(get_db), budget_id: int) -> Any:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to retrieve budget: {str(e)}"
        )
